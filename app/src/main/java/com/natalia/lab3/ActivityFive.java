...
public class ActivityFive extends AppCompatActivity {
    ...
    public  double  mistake(double param, int n, int marker){
        ...
        double p = lagrange.lagrange(param, 2, n);
        double p1 = lagrange.lagrange(param, 2, n+1);
        double p2 = lagrange.lagrange(param, 2, n+2);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        for(int i = 0; i <x.size(); i++){
            entries.add(new Entry(Float.parseFloat(x.get(i).toString()),
                    Float.parseFloat(y2.get(i).toString())));
        }
        LineDataSet dataset = new LineDataSet(entries, "F(x)");
        LineData data = new LineData(dataset);

        LineChart Graphf = (LineChart) findViewById(R.id.Graphf);
        ...
        LineChart Graphblurf = (LineChart) findViewById(R.id.Graphblurf);
        LineData data2 = new LineData(dataSets);
        Graphblurf.setData(data2);
        Graphblurf.invalidate();
